...
class RoborockZeoProtocol(RoborockEnum):
    ...
    UNKNOWN_243 = (
        243  # int, present in MQTT push from some devices, increments with each push
    )
    ...
    VOICE_VOLUME = 10009  # wo  [independent]  setVoiceVolume(int) → JSON.stringify({snd_volume: int})
    RPC_REQUEST = 10101  # wo  rpcRequest(method) → JSON
    RPC_RESPONSE = 10102  # -- MQTT push protocol 102, not a device DP
    VOICE_SWITCH = 10301  # wo  [independent]  setVoiceSwitchStatus(bool) → JSON.stringify({speech_switch: 1/0})
